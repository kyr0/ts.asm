...
class PluginX86 extends Plugin {
  onAsm(asm) {
    asm.hooks.command.tap('PluginX86', (name, args) => {
      switch (name) {
        case 'align':
          return this.align(...args)
        case 'lock':
          return asm._('tpl', TemplateX86Lock)
        case 'rex':
          return asm._('tpl', TemplateX86Rex, args)
        case 'mem':
        case 'disp':
          return this.mem(args[0])
        case 'imm':
          return this.imm.apply(this, args)
        default: {
  ...
  mem(disp: number | number64): MemoryX86 {
    if (typeof disp === 'number') return MemoryX86.factory(this.asm.opts.addressSize).disp(disp as number)
    else if (disp instanceof Array && disp.length == 2)
      return MemoryX86.factory(this.asm.opts.addressSize).disp(disp as number64)
    else throw TypeError('Displacement value must be of type number or number64.')
  }
