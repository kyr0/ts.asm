...
function isNumberOfDoubles(doubles, num) {
  ...
  for (let j = 0; j < doubles; j++) if (typeof num[j] !== 'number') return false
  return true
}
...
export class Constant extends Operand {
  ...
  setValue(value: number | number64) {
    ...
    } else throw TypeError(`Constant value must be of type TNumber, "${String(value)}" given.`)
  }
  ...
  zeroExtend(size) {
    ...
    let missing_bytes = (size - this.size) / 8
    this.size = size
    for (let i = 0; i < missing_bytes; i++) this.octets.push(0)
  }
  ...
  toString() {
    let str = ''
    for (let i = this.octets.length - 1; i >= 0; i--) {
      let oct = this.octets[i]
...
export class Immediate extends Constant {
  ...
  static factory(size, value: number | number64 = 0, signed = true) {
    switch (size) {
      case SIZE.B:
        return new Immediate8(value, signed)
      case SIZE.W:
        return new Immediate16(value, signed)
      case SIZE.D:
        return new Immediate32(value, signed)
      case SIZE.Q:
        return new Immediate64(value, signed)
      default:
        return new Immediate(value, signed)
    }
  }

  static throwIfLarger(value, size, signed) {
    let val_size = signed ? Constant.sizeClass(value) : Constant.sizeClassUnsigned(value)
...
export class Relative extends Variable {
  ...
  rebaseOffset(new_target: Expression): number {
    ...
    if (new_target.offset === -1) throw Error('Expression has no offset, cannot rebase.')
  ...
  toString() {
    ...
    if (this.target instanceof require('./Label').default) {
      ...
      // let lbl = this.target.asm.getStartLabel();
...
export class Operands {
  ...
  canEvaluate(owner: Expression): boolean {
    for (const op of this.list) {
      if (op instanceof Variable) if (!(op as Variable).canEvaluate(owner)) return false
    }
    return true
  }

  evaluate(owner: Expression) {
    for (const op of this.list) if (op instanceof Variable) (op as Variable).evaluate(owner)
  }
